...
namespace Gdd.Game.Engine.Render.Shadow
{
    ...
    /// </summary>
    public class GfxComponent : GameComponent
    {
        public GfxComponent(Microsoft.Xna.Framework.Game game, GraphicsDeviceManager graphics)
            : base(game)
        {
        ...
        public static RenderTarget2D CreateCustomRenderTarget(GraphicsDevice device, int numberLevels, SurfaceFormat surface, MultiSampleType type, int BackBufferWidth, int BackBufferHeight)
        {
            int width, height;

            // See if we can use our buffer size as our texture
            CheckTextureSize(BackBufferWidth, BackBufferHeight,
                out width, out height);
